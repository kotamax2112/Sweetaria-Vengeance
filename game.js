...
// --- PIXEL ART (SNES Style) ---
  ...
  // --- CLUES STATE ---
  SV.clues = Store.get('sv_clues', []);
  if (!SV.progress) SV.progress = {};
  if (SV.progress.cluesUnlocked == null) SV.progress.cluesUnlocked = false;
...
// --- INIT ---
  function init(){
    ...
    // Clues hotkey: C key when unlocked & not paused
...
// --- GAME LOOP ---
  ...
  function draw(){
    ...
    for(let i=0;i<5;i++){ const offset = ((t*40)+(i*60)) % (1000); ctx.fillRect(-offset+100,120+i*24, 260,12); }

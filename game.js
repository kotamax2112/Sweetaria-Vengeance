...
(() => {
  ...
  function drawCharacterSprite(ctx, x,y, opts){
    ...
    if(hairStyle === 'short'){
      ...
      for(let i=0;i<5;i++){ ctx.fillRect(x+8+i*4, y-26, 3, 6); }
      ...
      for(let i=0;i<6;i++){ ctx.fillRect(x+10+i*3, y-24, 2, 10); }
    }
  ...
  function buildItemOptions(){
    ...
    items.forEach(btn=>{
      ...
      if(item === SV.settings.item) btn.classList.add('primary');
      else btn.classList.remove('primary');
      btn.onclick = ()=>{
  ...
  function buildSkinsGrid(){
    ...
    skins.forEach(skin=>{
      ...
      if(!locked){
        ...
        card.onclick = ()=>{
          if(SV.settings.skin === skin.id) SV.settings.skin = null;
          else SV.settings.skin = skin.id;
          store.set('sv_settings', SV.settings);
          buildSkinsGrid();
          drawWardrobePreview();
        };
    ...
    if(saveBtn) saveBtn.onclick = ()=>{ closePopup(SV.popups.wardrobe); };
  ...
  function buildCluesList(){
    ...
    if(unlocked.final) entries.push({ label:'Final Clue', text:CLUES.final });
  ...
  function drawLoreCanvases(){
    ...
    if(c1){
      ...
      g.addColorStop(0, 'rgba(160,100,255,0.4)');
    ...
    if(c2){
      ...
      g.addColorStop(0, 'rgba(255,100,50,0.5)');
      g.addColorStop(1, 'transparent');
      ctx.fillStyle = g;
      ctx.fillRect(0,0,w,h);
  ...
  function doJump(){
    ...
    if(p.onGround || p.jumpsUsed < maxJumps){
      playSfx(SFX.jump);
      if(p.jumpsUsed === 0) p.vy = SV.jumpStrength;
      else p.vy = SV.doubleJumpStrength;
      p.onGround = false;
      p.jumpsUsed++;
  ...
  function setupRpgOverlay(){
    ...
    SV.rpg.buttons.forEach(btn=>{ btn.onclick = null; });
  }
  ...
  function showRpgOverlay(){
    if(SV.rpg.element) SV.rpg.element.classList.remove('hidden');
  }
  function hideRpgOverlay(){
    if(SV.rpg.element) SV.rpg.element.classList.add('hidden');
  }
  ...
  function handleRpgChoice(data){
    ...
    if(SV.rpg.hp <= 0){
      ...
      if(SV.rpg.kindOnly) awardAchievement('kind_only');
      SV.progress.lastCheckpoint = 10;
      store.set('sv_progress', SV.progress);
      SV.rpg.active = false;
      setTimeout(()=>{
        hideRpgOverlay();
  ...
  function clearRpgLog(){
    if(SV.rpg.logEl) SV.rpg.logEl.innerHTML = '';
  }
  ...
  function updateHazards(dt){
    ...
    if(SV.hazardSpawnMs >= SV.nextHazardIn){
      SV.hazardSpawnMs = 0;
      SV.nextHazardIn = randRange(900, 1600);
      spawnHazard();
    }
  ...
  function spawnHazard(){
    ...
    if(type === 'fire'){
      h.h = 34; h.y = SV.groundY - h.h; h.speedMul = 1.1;
    }else if(type === 'spike'){
      h.w = 30; h.h = 26; h.y = SV.groundY - h.h; h.speedMul = 1.15;
    }else{
      h.speedMul = 1.05;
    }
    SV.hazards.push(h);
  ...
  function spawnBoss1Projectile(){
    const lanesY = [ SV.groundY - 36, SV.groundY - 100, SV.groundY - 170 ];
    const y = pick(lanesY);
    SV.hazards.push({
      type:'emoji', x: SV.canvas.width + 40, y, w:34, h:34, damage:1, speedMul:1.05
    });
    SV.boss1ProjCount++;
  }
  ...
  function handleCollisions(dt){
    ...
    for(let i=SV.hazards.length-1;i>=0;i--){
      ...
      if(SV.laserMs>0 && SV.level !== 10){
        if(laserHitsHazard(h)){ SV.hazards.splice(i,1); continue; }
      }
      if(invSafe) continue;
  ...
  function onPlayerDeath(){
    ...
    awardAchievement('die_lot'); // simple counter logic not fully implemented but trigger exists
    ...
    if(dPopup){
      ...
      if(restartChk) restartChk.onclick = ()=>{ 
        closePopup(dPopup); 
        const lv = SV.progress.lastCheckpoint || 1;
        startStoryRun(lv, false); 
      };
      if(restartBeg) restartBeg.onclick = ()=>{
        closePopup(dPopup);
        startStoryRun(1, true);
      };
      if(exitMain) exitMain.onclick = ()=>{
        closePopup(dPopup);
        stopGameToHome();
      };
    }else{
  ...
  function drawGame(){
    ...
    if(SV.laserMs>0 && SV.level!==10){
      const p = SV.player;
      const x0 = p.x + p.w, x1 = w+10;
      let y0,y1;
      if(p.onGround){ y0 = p.y+6; y1 = y0 + LASER_THICKNESS_GROUND; }
      else{ y0 = p.y+2; y1 = y0 + LASER_THICKNESS_AIR; }
